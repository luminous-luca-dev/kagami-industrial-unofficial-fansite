		...
		try {
			...
			if (!error && data) {
				...
				document.getElementById('user-preview').innerText =
					currentUser.username || '名無し社員';
				document.getElementById('current-employee-id').innerText =
					maskedId;
			} else {
				document.getElementById('user-preview').innerText =
					'名無し社員';
				document.getElementById('current-employee-id').innerText =
					maskPlayerId(savedPlayerId);
				currentUser = {
