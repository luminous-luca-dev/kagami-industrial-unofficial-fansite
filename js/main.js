	...
	(e) => {
		try {
			...
			Promise.resolve().then(() => {
				console.log(
					'link-debug post-click defaultPrevented=',
					e.defaultPrevented,
					'target=',
					a.outerHTML
				);
...
document.addEventListener(
	...
	(e) => {
		try {
			const a =
				__lastTouchAnchor &&
				(__lastTouchAnchor.contains(e.target) ||
					__lastTouchAnchor === e.target)
					? __lastTouchAnchor
...
function initHeader() {
	...
	if (hamburger && mobileOverlay) {
		const toggleMobileMenu = (e) => {
			...
			document.body.style.overflow = mobileOverlay.classList.contains(
				'active'
			)
				? 'hidden'
				: '';
...
function highlightActiveNav() {
	...
	document
		.querySelectorAll('.main-nav__item a, .mobile-nav__list a')
		.forEach((link) => {
...
function triggerGoldenSign() {
	...
	setTimeout(() => {
		...
		setTimeout(() => {
			overlay.remove();
			document
				.querySelectorAll('.gold-particle')
				.forEach((p) => p.remove());
		}, 500);
	}, 3000);
...
function animateCounters() {
	document.querySelectorAll('[data-count]').forEach((el) => {
		...
		function update(now) {
			...
			el.textContent =
				Math.floor(target * eased).toLocaleString() + suffix;
