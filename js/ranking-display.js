...
(function () {
  ...
  async function fetchGlobalRanking() {
    try {
      const { data: players, error } = await _supabase.from('ranking').select('name, score, rank, created_at').order('score', { ascending: false }).order('created_at', { ascending: false }).limit(15);
      ...
      if (players.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" style="padding:20px; text-align:center;">登録者がいません。</td></tr>';
        return;
      }
      ...
      const tbody = document.getElementById('global-ranking-body');
      if (tbody) tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; color:#ff4a4a;">データ取得失敗</td></tr>';
    }
  }
