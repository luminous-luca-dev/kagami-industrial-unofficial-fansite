...
(function () {
  ...
  function initCandleHover() {
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      const sign = change >= 0 ? '+' : '';
      tooltipEl.innerHTML = `<div class="ct-label">${formatLabel(d.date, currentTf)}</div>` + `<div>始値 <b>${fmtKgm(d.o)}</b></div>` + `<div>高値 <b>${fmtKgm(d.h)}</b></div>` + `<div>安値 <b>${fmtKgm(d.l)}</b></div>` + `<div>終値 <b>${fmtKgm(d.c)}</b></div>` + `<div class="${change >= 0 ? 'ct-up' : 'ct-down'}">${sign}${fmtKgm(change)} KGM</div>`;
  ...
  function createCandleTooltip() {
    ...
    el.id = 'candleTooltip';
    el.style.cssText = 'position:absolute;display:none;background:rgba(10,18,28,0.95);' + 'border:1px solid rgba(200,166,94,0.25);color:rgba(255,255,255,0.78);' + 'font-size:11px;font-family:Montserrat,sans-serif;padding:8px 12px;' + 'border-radius:3px;pointer-events:none;z-index:100;line-height:1.7;' + 'min-width:130px;';
    el.innerHTML += '<style>.ct-label{color:#c8a65e;font-weight:700;margin-bottom:2px}' + '.ct-up{color:#c8a65e;font-weight:600}.ct-down{color:#7aadd6;font-weight:600}</style>';
  ...
  function renderChart(data, type) {
    ...
    const scaleOpts = {
      x: {
        ...
        ticks: { color: 'rgba(255,255,255,0.32)', maxTicksLimit: 8, font: { family: "'Montserrat', sans-serif", size: 9 } },
      },
      y: {
        position: 'right',
        grid: { color: 'rgba(255,255,255,0.04)' },
        border: { color: 'rgba(255,255,255,0.07)' },
        ticks: { color: 'rgba(255,255,255,0.32)', font: { family: "'Montserrat', sans-serif", size: 9 }, callback: (v) => fmtKgm(v) },
      },
    };
  ...
  function simulateTick(symbol) {
    ...
    const logR = (stock.drift - 0.5 * stock.vol * stock.vol) * dt + stock.vol * Math.sqrt(dt) * z;
  ...
  function updateHistoryTable() {
    ...
    const rows = [...state.history].reverse().map((r) => {
      ...
      const badge = isBuy ? '<span class="trade-badge trade-badge--buy">買</span>' : '<span class="trade-badge trade-badge--sell">売</span>';
      let pnlCell = '--';
      if (!isBuy && r.pnl != null) {
        const sign = r.pnl >= 0 ? '+' : '';
