...
async function generateIdCard(name) {
	...
	const regChoiceObj = document.querySelector(
		'input[name="registration-choice"]:checked'
	);
	const isRegistered = regChoiceObj ? regChoiceObj.value === 'yes' : false;

	// 2. 部署・役職・社員番号をここで確定させる（上書き防止！）
	const dept = isPresident
		? '加賀美インダストリアル'
		: getRandomItem(ID_CONFIG.departments);
	const rank = isPresident
		? '代表取締役社長'
		: getRandomItem(ID_CONFIG.ranks);
